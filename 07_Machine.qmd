...
df_7003 <- read_csv("ProcessedMachineData/7003_processed_data.csv", show_col_types = FALSE)
df_7011 <- read_csv("ProcessedMachineData/7011_processed_data.csv", show_col_types = FALSE)
df_7012 <- read_csv("ProcessedMachineData/7012_processed_data.csv", show_col_types = FALSE)
df_7013 <- read_csv("ProcessedMachineData/7013_processed_data.csv", show_col_types = FALSE)
TOPIX <- read_csv("^tpx_y.csv", show_col_types = FALSE)
```

```{r}
# 三井E&S
Net_Income_7003 <- df_7003 %>%
  filter(項目 %in% c("当期純利益", "当期純利益又は当期純損失（△）")) %>%
  slice(which(row_number() %% 2 != 0)) %>%
  select(No, `金額（百万円）`)

Net_assets_7003 <- df_7003 %>%
  filter(項目 %in% c("株主資本合計", "その他の包括利益累計額合計")) %>%
  group_by(No) %>%
  summarise(自己資本 = sum(`金額（百万円）`), .groups = 'drop')
# 三菱重工
Net_Income_7011 <- df_7011 %>%
  filter(項目 %in% c("当期純利益", "当期純利益又は当期純損失（△）")) %>%
  slice(which(row_number() %% 2 != 0)) %>%
  select(No, `金額（百万円）`)

Net_assets_7011 <- df_7011 %>%
  filter(項目 %in% c("株主資本合計", "その他の包括利益累計額合計")) %>%
  group_by(No) %>%
  summarise(自己資本 = sum(`金額（百万円）`), .groups = 'drop')
# 川崎重工
Net_Income_7012 <- df_7012 %>%
  filter(項目 %in% c("当期純利益", "当期純利益又は当期純損失（△）")) %>%
  slice(which(row_number() %% 2 != 0)) %>%
  select(No, `金額（百万円）`)

Net_assets_7012 <- df_7012 %>%
  filter(項目 %in% c("株主資本合計", "その他の包括利益累計額合計")) %>%
  group_by(No) %>%
  summarise(自己資本 = sum(`金額（百万円）`), .groups = 'drop')

# IHI
Net_Income_7013 <- df_7013 %>%
  filter(項目 %in% c("当期純利益", "当期純利益又は当期純損失（△）")) %>%
  slice(which(row_number() %% 2 != 0)) %>%
  select(No, `金額（百万円）`)

Net_assets_7013 <- df_7013 %>%
  filter(項目 %in% c("株主資本合計", "その他の包括利益累計額合計")) %>%
  group_by(No) %>%
  summarise(自己資本 = sum(`金額（百万円）`), .groups = 'drop')
...
# C0（最も直近の自己資本簿価）の定義
C0_7003 <- tail(Net_assets_7003$自己資本, 1)
C0_7011 <- tail(Net_assets_7011$自己資本, 1)
C0_7012 <- tail(Net_assets_7012$自己資本, 1)
C0_7013 <- tail(Net_assets_7013$自己資本, 1)
```
